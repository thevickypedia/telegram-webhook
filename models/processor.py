...
from models.settings import Message


def serialize(payload: Dict[str, Union[str, int, dict]]) -> Tuple[str, int]:
    base_class = Message(**payload)
    chat_payload = base_class.Chat(**{**payload['chat'], **payload['from']})
    if payload.get('video'):
        return process_video(chat_payload, base_class.Video(**payload['video']))
    if payload.get('text'):
        return process_text(chat_payload, base_class.Text(**payload))
    if payload.get('voice'):
        return process_voice(chat_payload, base_class.Voice(**payload['voice']))
    if payload.get('audio'):
        return process_audio(chat_payload, base_class.Audio(**payload['audio']))
    if payload.get('document'):
        return process_document(chat_payload, base_class.Document(**payload['document']))
    if payload.get('photo'):
        # Matches for compressed images
        return process_photo(chat_payload, [base_class.PhotoFragment(**d) for d in payload['photo']])
...
def process_text(chat: Message.Chat, data: Message.Text) -> Tuple[str, int]:
